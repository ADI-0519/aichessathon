...
| Search V10 selective bundle | Independently implemented dynamic NMP with deep verification, reverse futility, late-move and quiet-futility pruning, shallow capture SEE pruning, and contextual LMR. Its all-off `current` profile exactly reproduces current move, score, depth, nodes, qnodes, TT and LMR counts on Round 79 at 100k/300k/1M. Full V10 avoided `f3` at all limits, reached depth 10 at 1M versus current depth 9, and reduced qsearch share from 82.9% to 66.3%. On the six-position priority suite it retained three 300k reference hits, exchanging one current hit for one previously missed hit. | Promising high-impact challenger, not promoted. Run a technical smoke and a predeclared paired screen before adding TT buckets, singular extensions, or richer histories. |
