...
| Search V10 selective bundle | Independently implemented dynamic NMP with verification from depth 8, reverse futility, late-move and quiet-futility pruning, shallow capture SEE pruning, and contextual LMR. A safety review made PV classification use the entry window, protected killers and high-history quiets from shallow pruning, allowed protected late moves to remain unreduced, and made quiet futility request its own static evaluation. Its all-off `current` profile exactly reproduces current move, score, depth, nodes, qnodes, TT and LMR counts on Round 79 at 100k/300k/1M. The reviewed bundle reaches depth 10 at 1M versus current depth 9, but still selects the inferior `f3` at that cutoff. On the six-position priority suite it finds four of six references at 300k versus three for both current and the original aggressive V10. It scored 24W 10D 16L over 25 development pairs against current (58.0%, about +56 Elo) and independently 24W 11D 15L over 25 validation pairs (59.0%, about +63 Elo). Both runs had zero failures and positive results with each colour; each individual 95% interval remains wide. | Strength replicated on an untouched split. Promote after one clean colour-swapped official-clock safety pair, packaging gates, and archive inspection. |
| Search V10 S1 lean | Frozen reviewed V10 plus independently switchable signed bounded history and shallow quiet SEE. History rewards a quiet cutoff and penalizes only quiet alternatives actually searched before it; quiet SEE preserves first moves, checks, castling, killers, and high-history moves. `v10`, `history-v2`, and `quiet-see` profiles retain causal controls. | Implemented with static and focused unit gates passing. Await V10 validation; then require V10-profile parity and isolated fixed-node measurements before timed games. |
| KingNet V11-256 mixed80m | Format-3 evaluator trained for 16 epochs from 80M unique mixed positions; best validation objective was 0.007594 at epoch 16. At 10+0.1, the 75% blend scored 13W 7D 20L (41.25%, about -61 Elo) and the pure 100% evaluator scored 11W 12D 17L (42.5%, about -53 Elo) against current over the same 20 development pairs. Both had zero failures. | Reject both runtime candidates. Pure V11 recovered only half a game relative to V11-75, so the old HCE blend is not the principal cause of the search regression. Preserve training artifacts; do not spend the final day on another blend sweep. |
| V12 release | Search V10 plus signed history/quiet SEE, adaptive completed-iteration timing, stale king-bucket accumulator repair, and post-pruning qsearch accumulator updates. It passed incremental-update, two-colour smoke and packaging gates, then served as the frozen direct baseline for V14. | Promoted, released, and retained as the rollback champion. |
...
| V15 search family | Three explicit arms derived from frozen V14 Runtime: a bundled two-slot/2^22-entry main TT (`exp_release_v15_tt2`), TT2 plus bounded capture history/countermoves (`exp_release_v15_order`), and the full ordering build plus conservative single-ply singular extensions (`exp_release_v15_search`). Each mechanism is independently implemented and instrumented; qTT, ProbCut, singular multicut and double extensions are excluded. Reverse futility is forbidden in singular excluded-move verification. Unit, lint, type and cold-compile smokes pass; the corrected full build warmed in 75.7s locally. | Await paired games. Keep `current/` frozen until an arm wins and clears the 90s init gate with margin; the full build's local margin is narrow. |
