...
# Adaptive time control. The flat allocation gives a move where the search keeps
# changing its mind exactly as long as one it settled at depth 4, which is how
# two rated games were lost on moves that got the cap and no more.
ENABLE_ADAPTIVE_TIME = True
TIME_INSTABILITY_DROP = 60
TIME_SETTLED_DEPTHS = 4
TIME_SETTLED_MARGIN = 15
TIME_EARLY_EXIT_FRACTION = 0.35
TIME_EARLY_EXIT_MIN_DEPTH = 6
# Root moves change often at shallow depth; that is iterative deepening working,
# not the position being hard. Only a late change is worth paying for.
TIME_EXTEND_MIN_DEPTH = 7
TIME_EXTEND_MIN_CLOCK_MS = 25_000
...
def search_position(
    ...
    *,
    ...
    extended = False
    adaptive = (
        ENABLE_ADAPTIVE_TIME
        and time_limit_s is not None
        and hard_limit_s is not None
        and hard_limit_s > time_limit_s
    )
    # Never buy extra thinking out of a clock that is already short.
    may_extend = adaptive and clock_left_ms >= TIME_EXTEND_MIN_CLOCK_MS
    try:
        for depth in range(1, max_depth + 1):
            if time_limit_s is not None:
                elapsed_now = time.perf_counter() - started
                limit_now = hard_limit_s if (adaptive and extended) else time_limit_s
                if elapsed_now >= limit_now:
                    stopped = True
                    break
                # The move has not moved for several depths and the score is
                # steady: further depth is unlikely to change it, so bank the
                # rest of the budget for a position that needs it.
                if (
                    adaptive
            ...
            if score <= alpha or score >= beta:
                ...
                if aborted:
                    stopped = True
                    break
            changed = completed_depth > 0 and move != best_move
            dropped = completed_depth > 0 and score < best_score - TIME_INSTABILITY_DROP
            if changed or dropped:
                settled_depths = 0
                # The search changed its mind, or the position got worse than
                # the last depth believed. Buy the extra time once.
                # Only a genuine drop buys time. A root move that changes at
                # equal score is ordering churn, and this search produces plenty
                # of it; paying for that spends the clock on nothing.
                if (
                    dropped
                    and may_extend
                    and not extended
                    and depth >= TIME_EXTEND_MIN_DEPTH
                ):
                    extended = True
                    if timer is not None:
                        timer.cancel()
                        timer.join()
                    remaining_s = hard_limit_s - (time.perf_counter() - started)
                    if remaining_s > 0:
                        stop[0] = np.uint8(0)
                        timer = threading.Timer(
                            remaining_s, stop.__setitem__, args=(0, np.uint8(1))
                        )
                        timer.daemon = True
                        timer.start()
            elif completed_depth > 0 and abs(score - best_score) <= TIME_SETTLED_MARGIN:
                settled_depths += 1
            best_move = move
            best_score = score
