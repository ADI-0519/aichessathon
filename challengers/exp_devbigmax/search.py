    ...
    for _lmr_index in range(1, LMR_TABLE_MAX):
        _base_reduction = int(
            ...
            * np.log(float(_lmr_index))
            / 1.75  # dev's schedule: later moves reduced harder than V14's 2.25
        )
...
@njit(cache=False)
def _king_can_move(pieces: NDArray[np.uint64], side: int) -> bool:
    """True when the side to move has a legal king step, which rules out stalemate.

    Only valid when not in check: a king that is not attacked sits on no enemy
    slider's line, so stepping off its square cannot uncover an attack on the
    destination, and the ordinary attack test on that square is exact.
    Ported from the dev branch (aa32bf8).
    """
...
def _quiescence(
    ...
    # Deferred capture generation (dev aa32bf8).  Stand-pat below needs no move
    # list; generate_legal_captures only has to run first to detect stalemate
    # (it returns -1 when there is no legal move at all), and a king with a
    # legal step rules that out.  Otherwise build the list after stand-pat.
    count = -1
    if in_check:
    ...
    elif not _has_non_pawn_material(pieces, side) or not _king_can_move(pieces, side):
...
def _negamax(
    ...
    side = int(state[engine.STATE_SIDE])
    in_check = engine.is_in_check(pieces, side)
    # Deferred move generation (dev aa32bf8).  Generating every legal move
    # dominates node cost, and the TT, rule-draw, reverse-futility and
    # null-move cutoffs below never read the list; generating first only
    # matters for scoring a node with no legal moves.  Do it eagerly when that
    # cannot be ruled out cheaply -- in check, king and pawns only, or a king
    # with no legal step -- and otherwise only once those cutoffs have failed.
    count = -1
    if (
        in_check
        or not _has_non_pawn_material(pieces, side)
        or not _king_can_move(pieces, side)
    ):
        count = engine.generate_legal_moves(
            pieces,
            state,
