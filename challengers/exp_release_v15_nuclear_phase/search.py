...
RFP_RICH_MAX_DEPTH = 6
RFP_RICH_MARGIN_BASE = 40
RFP_RICH_MARGIN_PER_DEPTH = 80
RFP_PHASE_MIN_PIECES = 12
RFP_PHASE_MAX_PIECES = 28
V10_QF_MAX_DEPTH = 3
...
@njit(cache=False, inline="always")
def _phase_rfp_parameters(piece_count: int) -> tuple[int, int, int]:
    """Interpolate from conservative endgame to aggressive rich-position RFP."""
    phase_span = RFP_PHASE_MAX_PIECES - RFP_PHASE_MIN_PIECES
    phase = min(
        phase_span,
        max(0, piece_count - RFP_PHASE_MIN_PIECES),
    )
    max_depth = V10_RFP_MAX_DEPTH + (
        (RFP_RICH_MAX_DEPTH - V10_RFP_MAX_DEPTH) * phase // phase_span
    )
    margin_base = V10_RFP_MARGIN_BASE + (
        (RFP_RICH_MARGIN_BASE - V10_RFP_MARGIN_BASE) * phase // phase_span
    )
    margin_per_depth = V10_RFP_MARGIN_PER_DEPTH + (
        (RFP_RICH_MARGIN_PER_DEPTH - V10_RFP_MARGIN_PER_DEPTH)
        * phase
        // phase_span
    )
    return max_depth, margin_base, margin_per_depth
...
def _negamax(
    ...
    piece_count = engine.popcount(engine.all_occupancy(pieces))
    rfp_max_depth, rfp_margin_base, rfp_margin_per_depth = (
        _phase_rfp_parameters(piece_count)
    )
    null_move_candidate = (
        allow_null
        and depth >= 3
    ...
    if (
        ...
        and non_pv
        and (
            (ENABLE_V10_REVERSE_FUTILITY and depth <= rfp_max_depth)
    ...
    if (
        ...
        rfp_margin = rfp_margin_base + rfp_margin_per_depth * depth
