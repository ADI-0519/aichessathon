...
def _append_slider_captures(
    ...
    """Append the first enemy blocker on each ray, preserving full-generator order."""
    from_file = from_square & 7
    from_rank = from_square >> 3
    for direction in range(len(file_deltas)):
        file_index = from_file + int(file_deltas[direction])
        ...
        while 0 <= file_index < 8 and 0 <= rank < 8:
            ...
            if own_occupied & target_bit:
                break
            if enemy_occupied & target_bit:
                moves[count] = pack_move(from_square, target, 0, FLAG_CAPTURE)
                count += 1
                break
            file_index += int(file_deltas[direction])
            rank += int(rank_deltas[direction])
...
@njit(cache=False)
def generate_pseudo_legal_tacticals(
    pieces: NDArray[np.uint64], state: NDArray[np.int64], moves: NDArray[np.int32]
) -> int:
    """Generate captures and promotions in their full-generator relative order."""
    side = int(state[STATE_SIDE])
    enemy = BLACK if side == WHITE else WHITE
    own_occupied = occupancy_for(pieces, side)
    enemy_occupied = occupancy_for(pieces, enemy)
    ...
    while knights:
        from_square = lsb_square(knights)
        knights ^= bit(from_square)
        targets = KNIGHT_ATTACKS[from_square] & enemy_occupied
        while targets:
            target = lsb_square(targets)
            targets ^= bit(target)
            moves[count] = pack_move(from_square, target, 0, FLAG_CAPTURE)
    ...
    if kings:
        from_square = lsb_square(kings)
        targets = KING_ATTACKS[from_square] & enemy_occupied
        while targets:
            target = lsb_square(targets)
            targets ^= bit(target)
            moves[count] = pack_move(from_square, target, 0, FLAG_CAPTURE)
    ...
    for direction in range(len(file_deltas)):
        ...
        while 0 <= file_index < 8 and 0 <= rank < 8:
            ...
            if occupied & bit(target):
                break
            move = pack_move(from_square, target, 0, 0)
            if _quiet_candidate_is_legal(
                pieces,
                state,
                key,
                move,
...
def has_any_legal_quiet_move(
    ...
    """Return on the first legal non-capture without materialising quiet moves."""
    side = int(state[STATE_SIDE])
    own_occupied = occupancy_for(pieces, side)
    enemy = BLACK if side == WHITE else WHITE
    occupied = own_occupied | occupancy_for(pieces, enemy)
    ...
    while pawns:
        ...
        if 0 <= forward < 64 and occupied & bit(forward) == 0:
            promotion_rank = 6 if side == WHITE else 1
            flags = FLAG_PROMOTION if rank == promotion_rank else 0
            promotion = int(PROMOTION_KINDS[0]) if flags else 0
            move = pack_move(from_square, forward, promotion, flags)
            if _quiet_candidate_is_legal(
                pieces,
                state,
                key,
            ...
            if rank == start_rank:
                double_target = from_square + (16 if side == WHITE else -16)
                if occupied & bit(double_target) == 0:
                    move = pack_move(from_square, double_target, 0, FLAG_DOUBLE_PAWN)
                    if _quiet_candidate_is_legal(
                        pieces,
                        state,
                        key,
    ...
    while knights:
        from_square = lsb_square(knights)
        knights ^= bit(from_square)
        targets = KNIGHT_ATTACKS[from_square] & ~occupied
        while targets:
            target = lsb_square(targets)
            targets ^= bit(target)
            move = pack_move(from_square, target, 0, 0)
    ...
    while bishops:
        from_square = lsb_square(bishops)
        bishops ^= bit(from_square)
        if _slider_has_legal_quiet(
            pieces, state, key, from_square, occupied, side, king_square, king_rays,
            in_check, BISHOP_FILE_DELTAS, BISHOP_RANK_DELTAS, scratch_undo,
            scratch_undo_key,
        ):
            return True
    ...
    while rooks:
        ...
        if _slider_has_legal_quiet(
            pieces, state, key, from_square, occupied, side, king_square, king_rays,
            in_check, ROOK_FILE_DELTAS, ROOK_RANK_DELTAS, scratch_undo,
            scratch_undo_key,
        ):
            return True
    ...
    if kings:
        from_square = lsb_square(kings)
        targets = KING_ATTACKS[from_square] & ~occupied
        while targets:
            target = lsb_square(targets)
            targets ^= bit(target)
            move = pack_move(from_square, target, 0, 0)
            if _quiet_candidate_is_legal(
                pieces, state, key, move, side, king_square, king_rays, in_check,
                scratch_undo, scratch_undo_key,
            ):
                return True
        for kingside in (True, False):
            if _can_castle(pieces, state, side, kingside):
                target = (
                    G1 if kingside else C1
                ) if side == WHITE else (
                    G8 if kingside else C8
                )
                move = pack_move(from_square, target, 0, FLAG_CASTLING)
                if _quiet_candidate_is_legal(
                    pieces, state, key, move, side, king_square, king_rays, in_check,
                    scratch_undo, scratch_undo_key,
                ):
...
def generate_legal_captures(
    ...
    # -1 not 0 (keeps stalemate distinct from a position with no tacticals)
    moving_side = int(state[STATE_SIDE])
    pseudo_count = generate_pseudo_legal_tacticals(pieces, state, pseudo_moves)
    king_square, king_rays, in_check = _king_legality_context(pieces, state)
    legal_count = 0
    for index in range(pseudo_count):
        move = int(pseudo_moves[index])
    ...
    if legal_count > 0:
        return legal_count
    if has_any_legal_quiet_move(
        pieces, state, key, scratch_undo, scratch_undo_key
    ):
        return 0
    return -1
