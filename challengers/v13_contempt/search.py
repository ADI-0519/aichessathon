...
# Contempt. Every draw in this engine has always scored exactly zero, so it
# repeats the moment the alternative evaluates at or below nothing. Twenty of
# our twenty-four rated games ended in checkmate, twelve of them ours to lose,
# which is a violent record for engine play.
#
# We cannot know who we are playing: the FEN is the only input and nothing
# survives a game. So contempt cannot depend on the opponent, but it can depend
# on the position, which is free. A draw rescues a game we are losing on
# material and squanders one we are winning. The bias is close to inert when
# ahead, because a won line already scores above any draw value we would offer,
# and active exactly where round 97's perpetual saved us a rook down.
ENABLE_CONTEMPT = True
CONTEMPT_BEHIND = 40      # a draw is worth this much when material is against us
CONTEMPT_AHEAD = 40       # ...and costs this much when it is with us
CONTEMPT_MARGIN = 120     # material swing before either applies, about a minor piece
...
def _quiescence(
    ...
    if engine.has_rule_draw(
        ...
        return _draw_score(pieces, int(state[engine.STATE_SIDE])), False
...
@njit(cache=False, inline="always")
def _material_balance(pieces: NDArray[np.uint64], side: int) -> int:
    """Middlegame material from the side to move's point of view."""
    other_side = engine.BLACK if side == engine.WHITE else engine.WHITE
    balance = 0
    for kind in range(engine.PIECE_KIND_COUNT):
        own = engine.popcount(pieces[engine.piece_index(side, kind)])
        theirs = engine.popcount(pieces[engine.piece_index(other_side, kind)])
        balance += (own - theirs) * int(MG_VALUE[kind])
    return balance


@njit(cache=False, inline="always")
def _draw_score(pieces: NDArray[np.uint64], side: int) -> int:
    """What a draw is worth here, from the side to move's point of view."""
    if not ENABLE_CONTEMPT:
        return 0
    balance = _material_balance(pieces, side)
    if balance <= -CONTEMPT_MARGIN:
        return CONTEMPT_BEHIND
    if balance >= CONTEMPT_MARGIN:
        return -CONTEMPT_AHEAD
    return 0
...
def _negamax(
    ...
    if count == 0:
        return (
            -MATE_SCORE + ply
            if in_check
            else _draw_score(pieces, int(state[engine.STATE_SIDE]))
        ), False
    if engine.has_rule_draw(
        pieces, state, key[0], history, history_count, root_history_count
    ):
