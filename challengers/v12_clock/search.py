...
# Easy-move detection. When the root move has not changed for several completed
# depths and the score is steady, more depth rarely changes the answer, so the
# rest of the budget is worth more to a later move than to this one. This is the
# banking half of the V9 experiment; its extension half is deliberately absent,
# because rounds 77 to 82 show the engine short of time rather than short of
# thinking, and extending would deepen exactly that.
ENABLE_TIME_BANKING = True
TIME_SETTLED_DEPTHS = 4
TIME_SETTLED_MARGIN = 15
TIME_EARLY_EXIT_FRACTION = 0.4
TIME_EARLY_EXIT_MIN_DEPTH = 6

# Compile-time search switches. Numba freezes module globals when it compiles,
# so these select a build rather than a runtime option, and tools/search_ablations
# can turn one off at a time to price it. Every one of them trades exactness for
# nodes: the search sees less, but it sees it sooner.
ENABLE_LMR_SCALING = True
ENABLE_REVERSE_FUTILITY = True
ENABLE_RAZORING = True
ENABLE_FUTILITY = True
ENABLE_LATE_MOVE_PRUNING = True
# This history table only ever grows (no maluses), so a low value means "never
# caused a cutoff" rather than "known bad" and hard history pruning has nothing
# to bite on. Maluses were measured at 40.0% and rejected, so history modulates
# the reduction instead of vetoing the move.
ENABLE_HISTORY_LMR = True
ENABLE_SEE_PRUNING = True

# Margins in centipawns. Chosen to be conservative: this engine's evaluation is
# noisier than the ones these techniques are usually tuned against, so a margin
# that is too tight throws away real moves.
...
SEE_PRUNING_MARGIN = -35


def _build_lmr_table() -> NDArray[np.int32]:
    """Late-move reductions grow with depth and with how late the move is.

    The flat one-ply reduction this replaces treats move 5 of 40 the same as
    move 39, and depth 3 the same as depth 12. The usual log-log shape spends
    the saved nodes where ordering says they are least likely to matter.
    """
    table = np.zeros((MAX_DEPTH + 1, engine.MAX_MOVES + 1), dtype=np.int32)
    for depth in range(1, MAX_DEPTH + 1):
        for move_index in range(1, engine.MAX_MOVES + 1):
            reduction = 0.75 + math.log(depth) * math.log(move_index) / 2.25
            table[depth, move_index] = max(0, int(reduction))
    return table
...
def _negamax(
    ...
    is_pv = beta - alpha != 1
    static_eval = 0 if in_check else evaluate(pieces, state, accumulator_stack[ply])

    # Reverse futility: so far ahead that even conceding the margin holds beta.
    if (
    ...
    # Razoring: so far behind at shallow depth that only a capture sequence can
    # rescue it, so ask quiescence directly instead of opening a full node.
    if (
        ENABLE_RAZORING
        and not is_pv
        and not in_check
    ...
    for index in range(count):
        ...
        # SEE pruning reads the position before the move, so it runs here and
        # skips the make/unmake entirely rather than after the fact.
        if (
            ENABLE_SEE_PRUNING
            and index > 0
            and not quiet
            ...
            and static_exchange_eval(pieces, state, move, see_gain_stack[ply])
            < SEE_PRUNING_MARGIN * depth
        ):
            continue
        nnue.update_for_move(
            pieces,
        ...
        # Quiet moves that neither give check nor plausibly reach alpha. Whether
        # a move gives check is only known once it is made, so these unmake and
        # move on; the saving is the subtree, not the make.
        if (
            index > 0
            and quiet
            and not is_pv
            ...
            and (
                (
                    ...
                    and index >= LATE_MOVE_PRUNING_BASE + depth * depth
                )
                or (
                    ENABLE_FUTILITY
        ...
        if depth >= 3 and index >= 4 and quiet and not in_check and not gives_check:
            if ENABLE_LMR_SCALING:
                reduction = int(
                    LMR_TABLE[
                        min(depth, MAX_DEPTH), min(index, engine.MAX_MOVES)
                    ]
                )
                ...
                if (
                    ...
                    and int(
                        quiet_history[
                            side, engine.move_from(move), engine.move_to(move)
                        ]
                    )
                    > 0
                ):
    ...
    try:
        for depth in range(1, max_depth + 1):
            if time_limit_s is not None:
                ...
                if elapsed_now >= time_limit_s:
                    stopped = True
                    break
                if (
                    ENABLE_TIME_BANKING
                    and settled_depths >= TIME_SETTLED_DEPTHS
                    and completed_depth >= TIME_EARLY_EXIT_MIN_DEPTH
                    and elapsed_now >= time_limit_s * TIME_EARLY_EXIT_FRACTION
                ):
                    break
            window = 45
            ...
            if completed_depth > 0 and (
                move != best_move or abs(score - best_score) > TIME_SETTLED_MARGIN
            ):
                settled_depths = 0
