...
# These flags are frozen into Numba's machine code during warm-up. They may be
# changed exactly once with ``configure_experiment`` before any search function
# compiles; production candidates should use a single named profile.
ENABLE_LMR = True
ENABLE_QSEARCH_PRUNING = True
...
def _quiescence(
    ...
    for index in range(count):
        ...
        if not in_check and not gives_check:
            losing_non_promotion = ENABLE_QSEARCH_PRUNING and (
                flags & engine.FLAG_PROMOTION == 0
                and int(score_stack[ply, index]) < 2_000_000
            )
            delta_fails = ENABLE_QSEARCH_PRUNING and (
                alpha > -MATE_BOUND
...
def _negamax(
    ...
    if depth <= 0:
        side = int(state[engine.STATE_SIDE])
        if extensions_used < MAX_CHECK_EXTENSIONS and engine.is_in_check(pieces, side):
            depth = 1
            extensions_used += 1
            extended_here = True
        else:
    ...
    side = int(state[engine.STATE_SIDE])
    in_check = engine.is_in_check(pieces, side)
    if in_check and not extended_here and extensions_used < MAX_CHECK_EXTENSIONS:
        depth += 1
        extensions_used += 1
    count = engine.generate_legal_moves(
        pieces,
...
_EXPERIMENT_PROFILES = {
    "baseline": (True, True, 0),
    "no-lmr": (False, True, 0),
    "no-q-pruning": (True, False, 0),
    "check-extension": (True, True, 1),
}


def available_profiles() -> tuple[str, ...]:
    """Return the named compile-time profiles supported by the V4 lab."""
    return tuple(_EXPERIMENT_PROFILES)


def configure_experiment(name: str) -> None:
    """Select one ablation before Numba compiles the recursive search.

    Reconfiguration after warm-up would appear to succeed while leaving old
    machine code active, so it is rejected explicitly.
    """
    global ACTIVE_PROFILE, ENABLE_LMR, ENABLE_QSEARCH_PRUNING, MAX_CHECK_EXTENSIONS
    ...
    ENABLE_LMR, ENABLE_QSEARCH_PRUNING, MAX_CHECK_EXTENSIONS = _EXPERIMENT_PROFILES[name]
    ACTIVE_PROFILE = name
