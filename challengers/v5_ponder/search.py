...
def search_position(
    ...
    *,
    ...
    stop: NDArray[np.uint8] | None = None,
) -> SearchResult:
    ...
    # A caller that wants to interrupt the search from another thread supplies its
    # own flag; the tree polls it every 256 nodes, so a ponder stops in microseconds.
    if stop is None:
