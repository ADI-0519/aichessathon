...
# Behaviour-changing LMR V2 experiment.  The shipped engine reduces every eligible
# late quiet by exactly one ply.  This schedule keeps that behaviour for shallow/early
# quiets but reduces much more aggressively as both depth and move number grow.
# PV nodes, killers, checks, tactical moves, and historically successful quiets are
# protected.  Any reduced move that raises alpha is re-searched at full depth below.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVE_INDEX = 4  # zero-based: fifth move
LMR_HISTORY_PROTECT = 64
...
def _lmr_reduction(
    ...
    """Return late-move reduction in plies for an already-eligible quiet move.

    The current champion always returns one here.  V9b allows a two-ply
    reduction for sufficiently deep/late quiets, while preserving more depth at
    PV nodes and for quiets that have previously produced beta cutoffs.
    """
    ...
    # V9b is deliberately moderate: the failed V9 staircase reached R=3/4 and
    # lost two previously solved reference moves at 1M nodes.  Keep the large
    # structural improvement (later quiets can be reduced by two plies), but
    # do not use reductions deeper than R=2 until games prove this search can
    # tolerate them.
    if depth >= 5 and move_number >= 8:
        reduction = 2

    # Preserve more depth in the positions where a reduced move is most likely
    # to matter: PV nodes and quiets with prior cutoff evidence.
    if pv_node and reduction > 1:
        reduction -= 1
    if history_score >= LMR_HISTORY_PROTECT and reduction > 1:
        reduction -= 1

    # Never reduce straight through the horizon: keep at least one full search ply
    # before quiescence.
    maximum = depth - 2
...
def _negamax(
    ...
    original_beta = beta
    pv_node = original_beta - original_alpha > 1
    ...
    for index in range(count):
        ...
        gives_check = engine.is_in_check(pieces, int(state[engine.STATE_SIDE]))
        reduction = 0
        if quiet and not in_check and not gives_check:
            from_square = engine.move_from(move)
            to_square = engine.move_to(move)
            history_score = int(quiet_history[side, from_square, to_square])
            is_killer = ply < MAX_PLY and (
