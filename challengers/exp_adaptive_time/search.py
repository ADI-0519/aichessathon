...
def _should_stop_after_iteration(
    *,
    ...
    """Decide whether another full iteration is worth starting.

    The hard deadline remains authoritative inside the compiled search. This
    policy only stops between completed iterations, where the current result is
    safe to return.
    """
    if (
    ...
    if elapsed_s >= normal_limit_s and not unstable:
        return True
    remaining_s = hard_limit_s - elapsed_s
    predicted_next_s = last_iteration_s * NEXT_ITERATION_COST_MULTIPLIER
    return remaining_s <= predicted_next_s
