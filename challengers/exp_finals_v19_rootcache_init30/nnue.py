...
class ModelData(TypedDict):
    ...
    accumulator_bias: NDArray[np.float32]
    accumulator: int
    pairwise_width: int
    piece_head_map: NDArray[np.int32]
    head_count: int
    hidden_size: int
    hidden: NDArray[np.float32]
    hidden_bias: NDArray[np.float32]
    output_relu: NDArray[np.float32]
    output_square: NDArray[np.float32]
