...
def _record_quiet_cutoff(
    ...
    quiet_history[side, from_square, to_square] = min(HISTORY_LIMIT, previous + bonus)
    if previous_move != 0:
        previous_from = engine.move_from(previous_move)
        previous_to = engine.move_to(previous_move)
        countermoves[side, previous_from, previous_to] = np.int32(move)


@njit(cache=False, inline="always")
def _penalize_failed_quiets(
    side: int,
    moves: NDArray[np.int32],
    cutoff_index: int,
    depth: int,
    ...
    """Apply a history malus to quiet moves tried before a quiet cutoff."""
    malus = depth * depth
    for index in range(cutoff_index):
        move = int(moves[index])
        flags = engine.move_flags(move)
        if flags & (engine.FLAG_CAPTURE | engine.FLAG_PROMOTION):
            continue
        from_square = engine.move_from(move)
        to_square = engine.move_to(move)
        previous = int(quiet_history[side, from_square, to_square])
        quiet_history[side, from_square, to_square] = max(
            -HISTORY_LIMIT, previous - malus
        )
...
def _negamax(
    ...
    previous_move = int(move_stack[ply - 1]) if ply > 0 else 0
    counter_move = 0
    if previous_move != 0:
        previous_from = engine.move_from(previous_move)
        previous_to = engine.move_to(previous_move)
        counter_move = int(countermoves[side, previous_from, previous_to])
