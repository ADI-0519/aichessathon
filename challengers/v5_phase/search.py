...
NNUE_BLEND = 50
# The piece-square network is least reliable in sparse endings.  Taper its
# contribution from NNUE_BLEND at full material to this floor as material
# disappears, leaving the existing tapered handcrafted evaluator in control.
NNUE_ENDGAME_BLEND = 15
...
def evaluation_phase(pieces: NDArray[np.uint64]) -> int:
    """Return the conventional 0--24 non-pawn material phase."""
    phase = 0
    for color in range(2):
        for kind in range(engine.KNIGHT, engine.QUEEN + 1):
            index = engine.piece_index(color, kind)
            phase += engine.popcount(pieces[index]) * int(PHASE_VALUE[kind])
    return min(phase, MAX_PHASE)


@njit(cache=False, inline="always")
def learned_blend_for_phase(phase: int) -> int:
    """Interpolate the learned-evaluation percentage over material phase."""
    bounded_phase = min(MAX_PHASE, max(0, phase))
    return (
        NNUE_ENDGAME_BLEND * (MAX_PHASE - bounded_phase)
        + NNUE_BLEND * bounded_phase
    ) // MAX_PHASE
...
def evaluate(
    ...
    """Blend V4's evaluator with the learned evaluator by material phase."""
    if NNUE_BLEND <= 0:
        return handcrafted_evaluate(pieces, state)
    learned = nnue.evaluate(accumulators, int(state[engine.STATE_SIDE]))
    blend = learned_blend_for_phase(evaluation_phase(pieces))
    if blend >= 100:
        return learned
    handcrafted = handcrafted_evaluate(pieces, state)
    return (blend * learned + (100 - blend) * handcrafted) // 100
